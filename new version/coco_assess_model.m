...
%of variables
imageNames = [".\images\test-resized\000000000285.jpg" ".\images\test-resized\000000000139.jpg"];
figure
tiledlayout(1,2)
images_predict = [];
labels_predict = [];
...
if investigate
    imageIdx = 1;
    testImage = images_predict{imageIdx};
    ...
    for i = 1:length(targetClasses)
        ...
        imshow(testImage)
        hold on
        title(targetClasses(i))
        imagesc(map(:,:,i),AlphaData=0.5)
        hold off
    end
    colormap jet
